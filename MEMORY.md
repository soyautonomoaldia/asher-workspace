...
- FinOps reasoning rule approved on 2026-07-05 for GPT-5.5: permanent default `reasoning_effort`/`thinkingDefault` is `medium` for Asher and AD agents. Use `none` only for mechanical status/formatting, `low` for summaries/light copy/simple cleanup, `medium` for normal domain work, `high` only for gates, audits, strategic/product/technical/commercial/privacy-sensitive reasoning, and `xhigh` only for irreversible or high-impact legal, financial, security, privacy, production, credentials, billing, real-data or data-loss risk. No agent should use `high` or `xhigh` as default.
- Token optimization is a standing priority in recommendations and delegation design. For work with agents, prefer clean OpenClaw chats/sessions for long prompts, document creation, analysis, audits, or multi-step tasks; use Discord mainly for short commands, status, gates, and links/summaries. Before advising Salva where to send an instruction, explicitly consider token cost, context bloat, and whether a clean session would be cheaper and clearer.
